        ...
        return(
            <View style={styles.class__container}>
                <View style={styles.response__header}>
                    ...
                    <Text style={[styles.text__header,{marginLeft: 16}]}>{this.state.username}</Text>
                </View>
                ...
                { this.state.sent ? 
                    ...
                    : <TextInput style={this.state.expand ? styles.multi: styles.answer__container}
                ...
                { this.state.expand ? 
                    ...
                    style={[styles.send__button, {backgroundColor: this.state.disabled ? '#bfbfbf' : '#89cf50'}]}>
...
export default class ResponseList extends Component {
    ...
    render(){
        return(
            <ScrollView>
                {this.state.responses.map(res => <Response response={res.response}
                username={res.user} response_id = {res.id} key={res.id}/>)}
            </ScrollView>
        );
    }
}